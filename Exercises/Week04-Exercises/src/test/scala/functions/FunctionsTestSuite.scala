...
class FunctionsTestSuite extends FunSuite {
  ...
  test("Calling tail on an empty list throws IllegalArgumentException") {
      val thrown = intercept[IllegalArgumentException] {
        tail(List())
      }
      assert(thrown.getMessage === "Cannot call tail on empty list")
   }
  ...
  test("setHead adds a value to the front of an empty list") {
    assert(setHead(Nil, "Ok") == "Ok" :: Nil )
  }
